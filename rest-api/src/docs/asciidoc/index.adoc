...
Given the url in the `Location` header that was returned from the previous response, we can add an expense to this nobt if we post an expense to the `/expenses` sub-resource.
